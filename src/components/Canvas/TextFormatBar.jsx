...
const btnBase =
    'h-7 min-w-7 px-1.5 flex items-center justify-center rounded text-xs font-semibold transition-colors';
const btnOff = `${btnBase} text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700`;
const btnOn = `${btnBase} bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200`;

const TextFormatBar = ({ object, anchor, onChange }) => {
    if (!object || !anchor) return null;

    const isTextType = object.type === 'text';
    const fontFamily = object.fontFamily || 'Inter, system-ui, sans-serif';
    ...
    return createPortal(
        <div
            className="fixed z-[1100] flex items-center gap-1 px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-lg"
            style={{ top, left }}
        ...
        >
            ...
            <select
                className="h-7 text-xs rounded border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 px-1 max-w-[120px] cursor-pointer"
                value={matchedFont}
            ...
            <select
                className="h-7 text-xs rounded border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 px-1 w-14 cursor-pointer"
                value={fontSize}
